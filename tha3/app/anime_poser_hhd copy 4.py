...
class ParameterStore:
    ...
    # 向后寻找匹配的key返回的是一个map
    def find_back_key(self,key_index,timeline_keys,lables):
        ret_map = {}
        current_key = timeline_keys[key_index]
        if lables[0] in self.morph_lable:
            for i in range(key_index+1,len(timeline_keys)):
                key = timeline_keys[i]
                if lables[0] in self.parameter_map[key] and lables[1] in self.parameter_map[key][lables[0]] and bool(self.parameter_map[key][lables[0]][lables[1]]):
                    type = self.parameter_map[current_key][lables[0]]["type"]
                    start = (current_key,self.parameter_map[current_key][lables[0]][lables[1]]["value"])
                    start_control = (self.parameter_map[current_key][lables[0]][lables[1]]["control"]["start"][0],self.parameter_map[current_key][lables[0]][lables[1]]["control"]["start"][1])
                    end = (key,self.parameter_map[key][lables[0]][lables[1]]["value"])
                    end_control = (self.parameter_map[key][lables[0]][lables[1]]["control"]["end"][0],self.parameter_map[key][lables[0]][lables[1]]["control"]["end"][1])
                    ret_map = {
                        "type":type,
                        "start":start,
            ...
            if not bool(ret_map):
                type = self.parameter_map[current_key][lables[0]]["type"]
                end = (self.frame_num+1,self.parameter_map[current_key][lables[0]][lables[1]]["value"])
                end_control = (0,0)
                start = (current_key,self.parameter_map[current_key][lables[0]][lables[1]]["value"])
                start_control = (0,0)
                ret_map = {
            ...
            for i in range(key_index+1,len(timeline_keys)):
                key = timeline_keys[i]
                if lables[0] in self.parameter_map[key] and bool(self.parameter_map[key][lables[0]]):
                    start = (current_key,self.parameter_map[current_key][lables[0]]["value"])
                    start_control = (self.parameter_map[current_key][lables[0]]["control"]["start"][0],self.parameter_map[current_key][lables[0]]["control"]["start"][1])
                    end = (key,self.parameter_map[key][lables[0]]["value"])
                    end_control = (self.parameter_map[key][lables[0]]["control"]["end"][0],self.parameter_map[key][lables[0]]["control"]["end"][1])
                    ret_map = {
                        "start":start,
                        "start_control":start_control,
                        "end":end,
            ...
            if not bool(ret_map):
                end = (self.frame_num+1,self.parameter_map[current_key][lables[0]]["value"])
                end_control = (0,0)
                start = (current_key,self.parameter_map[current_key][lables[0]]["value"])
                start_control = (0,0)
                ret_map = {
    ...
    # 向前寻找匹配的key返回的是一个map
    def find_front_key(self,key_index,timeline_keys,lables):
        ret_map = {}
        current_key = timeline_keys[key_index]
        if lables[0] in self.morph_lable:
            for i in range(key_index-1,-1,-1):
                key = timeline_keys[i]
                if lables[0] in self.parameter_map[key] and lables[1] in self.parameter_map[key][lables[0]] and bool(self.parameter_map[key][lables[0]][lables[1]]):
                    type = self.parameter_map[key][lables[0]]["type"]
                    start = (key,self.parameter_map[key][lables[0]][lables[1]]["value"])
                    start_control = (self.parameter_map[key][lables[0]][lables[1]]["control"]["start"][0],self.parameter_map[key][lables[0]][lables[1]]["control"]["start"][1])
                    end = (current_key,self.parameter_map[current_key][lables[0]][lables[1]]["value"])
                    end_control = (self.parameter_map[current_key][lables[0]][lables[1]]["control"]["end"][0],self.parameter_map[current_key][lables[0]][lables[1]]["control"]["end"][1])
                    ret_map = {
                        "type":type,
                        "start":start,
                        "start_control":start_control,
            ...
            if not bool(ret_map):
                type = self.parameter_map[current_key][lables[0]]["type"]
                start = (0,self.parameter_map[current_key][lables[0]][lables[1]]["value"])
                start_control = (0,0)
                end = (current_key,self.parameter_map[current_key][lables[0]][lables[1]]["value"])
                end_control = (0,0)
                ret_map = {
            ...
            for i in range(key_index-1,-1,-1):
                key = timeline_keys[i]
                if lables[0] in self.parameter_map[key] and bool(self.parameter_map[key][lables[0]]):
                    start = (key,self.parameter_map[key][lables[0]]["value"])
                    start_control = (self.parameter_map[key][lables[0]]["control"]["start"][0],self.parameter_map[key][lables[0]]["control"]["start"][1])
                    end = (current_key,self.parameter_map[current_key][lables[0]]["value"])
                    end_control = (self.parameter_map[current_key][lables[0]]["control"]["end"][0],self.parameter_map[current_key][lables[0]]["control"]["end"][1])
                    ret_map = {
                        "start":start,
                        "start_control":start_control,
                        "end":end,
            ...
            if not bool(ret_map):
                start = (0,self.parameter_map[current_key][lables[0]]["value"])
                start_control = (0,0)
                end = (current_key,self.parameter_map[current_key][lables[0]]["value"])
                end_control = (0,0)
                ret_map = {
    ...
    def find_keys(self):
        ...
        if key in self.parameter_map:
            ...
            for k,v in self.parameter_map[key].items():
                ...
                if k in self.morph_lable and bool(self.parameter_map[key][k]):
                    if "left" in self.parameter_map[key][k] and bool(self.parameter_map[key][k]["left"]):
                        lables = [k,"left"]
                        point_map = self.find_back_key(key_index=key_index,timeline_keys=timeline_keys,lables=lables)
                        if bool(point_map):
                            if k in function_map["start"] and bool(function_map["start"][k]):
                                ...
                                function_map["start"][k] = {"type":point_map["type"],
                                                            "left":{"start":point_map["start"],
                                                                    ...
                                                                    "start_control":point_map["start_control"],
                                                                    "end_control":point_map["end_control"]}}
                        point_map = self.find_front_key(key_index=key_index,timeline_keys=timeline_keys,lables=lables)
                        if bool(point_map):
            ...
            index=self.find_nearest_key(key,timeline_keys)
            if index>-1:
                pass
            else:
                pass
