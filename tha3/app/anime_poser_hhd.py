...
class TimelnePanel(wx.Panel):
    def __init__(self, main_panel, parent, timeline_width, timeline_height, slider_height, frame_num, timeline_gap, lable_len, lable_gap, data_map, item_list):
        ...
        self.point_nomal_size = 2
        self.point_selected_size = 4
        self.point_nomal_color = "red"
        self.point_selected_color = "green"
        self.data = data_map
        ...
        self.timeline_draw_panel.Bind(wx.EVT_LEFT_DOWN, self.on_timeline_draw_left_down)
        ...
        if bool(self.data.parameter_map):
            ...
            for key,value in self.data.parameter_map.items():
                for k in self.data.parameter_map[key]:
                    if (key,k) in self.data.selected_pointes:
                        self.timeline_draw_panel_dc.SetPen(wx.Pen(self.point_selected_color, self.point_selected_size))
                        self.timeline_draw_panel_dc.SetBrush(wx.Brush(self.point_selected_color))
                    else:
                        self.timeline_draw_panel_dc.SetPen(wx.Pen(self.point_nomal_color, self.point_nomal_size))
                        self.timeline_draw_panel_dc.SetBrush(wx.Brush(self.point_nomal_color))
    ...
    def timeline_slider_num2x(self,slider_num):
        position = slider_num/self.data.frame_num
    ...
    # 滑动滑块的回调函数
    def OnSlided(self, event):
        key = self.GetTimelineValue()
        self.data.set_now_key_value(key)
        self.SetAllSliderValue()
        self.SetTimeText(key)
        self.main_panel.Refresh() #刷新整个页面clear
        main_frame.slider_event_flage = True
        # self.timeline_draw_panel.Refresh()

    # 鼠标左键点击画板事件回调
    def on_timeline_draw_left_down(self,event):
        x, y = event.GetPosition()

        near_key = self.data.find_selected_point(click_x=x,
                                                 click_y=y,
                                                 slider_width=self.timeline_width-self.timeline_gap*2,
                                                 slider_gap=self.timeline_gap)
        
        if near_key is not None:
            self.data.set_selected_pointes(selected_point=near_key)
            self.timeline_slider.SetValue(self.data.get_selected_point_frame())
            wx.PostEvent(self.timeline_slider, wx.CommandEvent(wx.EVT_SLIDER.typeId, self.timeline_slider.GetId()))
            
        else:
            self.data.clear_selected_pointes()
            
        self.main_panel.curve_panel.on_timeline_draw_left_click()
    ...
    #设置指示器上的值
    def SetTimeText(self,key):
        sec_txt = str(key//self.main_panel.fps).zfill(2)
        fram_txt = str(key%self.main_panel.fps).zfill(2)
        self.main_panel.s_text_ctrl.SetValue(sec_txt)
        self.main_panel.z_text_ctrl.SetValue(fram_txt)
...
class DragPoint(wx.Window):
    ...
    def OnLeftUp(self, event):
        ...
        if self.dragging:
            ...
            self.ReleaseMouse()
            # 修改了控制点后更新参数空间的参数
            self.frame.update_control_point()
            # 设置拖动标志为False
            self.dragging = False
    ...
    def OnMotion(self, event):
        ...
        if self.dragging:
            ...
            self.SetPosition((self.x - 4, self.y - 4))
            # 更新贝塞尔曲线窗口对象中的控制点列表
            if len(self.frame.points)>self.index and self.frame.points[self.index] is not None:
...
class CurvePlottingPanel(wx.Panel):
    ...
    def __init__(self, parent,data_map):
        ...
        self.botton_gap = 15
        self.line_size = 1
        self.lable_right_gap = 100
        self.lable_text_size = 13
        self.point_size = 2
        self.line_size = 2
        self.control_point_size = 8
        self.lable_str = "NONE"

        self.start_control_dragpoint = None
        self.end_control_dragpoint = None

        self.selected_key = None
        self.selected_lable = None

        self.data = data_map
        self.point_frame_index = [] # 用于记录匹配到的关键帧index，如果有左右区分，第一个时左，第二个是右
        self.parameter_points = [] # 用于记录参数空间中的原始点的数据,如果是有左右之分的则len为8，前4个为left，后四个为right，如果没有左右之分，则len为4
        self.points = [] # 用于记录各个点在曲线面板中的坐标

        self.change_falge = False
    ...
    # 原始的点数据转换为曲线面板的点绘制数据
    def point_data_2_convert_data(self,start,start_control,end,end_control):
        ret_map={}
        height = self.height-self.botton_gap-self.top_gap
        width = self.width-self.left_gap
        y_axis = start[1]-end[1]
        y_axis_num = abs(y_axis)
        x_axis = start[0]-end[0]
        x_axis_num = abs(x_axis)
        y_scale = y_axis_num/height
        x_scale = x_axis_num/width
        ...
        if y_axis>0 :
            ret_map["start"]=(self.left_gap,self.top_gap)
            ret_map["end"]=(self.width,self.height-self.botton_gap)
            ret_map["start_control"]=(math.floor(start_control[0]/x_scale+ret_map["start"][0]),math.floor(start_control[1]/y_scale+ret_map["start"][1]))
            ret_map["end_control"]=(math.floor(end_control[0]/x_scale+ret_map["end"][0]),math.floor(end_control[1]/y_scale+ret_map["end"][1]))
        elif y_axis<0 :
            ret_map["start"]=(self.left_gap,self.height-self.botton_gap)
            ret_map["end"]=(self.width,self.top_gap)
            ret_map["start_control"]=(math.floor(start_control[0]/x_scale+ret_map["start"][0]),math.floor(start_control[1]/y_scale+ret_map["start"][1]))
            ret_map["end_control"]=(math.floor(end_control[0]/x_scale+ret_map["end"][0]),math.floor(end_control[1]/y_scale+ret_map["end"][1]))
        else:
            ret_map["start"]=(self.left_gap,self.top_gap + math.floor(height/2))
            ret_map["end"]=(self.width,self.top_gap + math.floor(height/2))
            ret_map["start_control"]=(math.floor(start_control[0]/x_scale+ret_map["start"][0]),math.floor(start_control[1]/y_scale+ret_map["start"][1]))
            ret_map["end_control"]=(math.floor(end_control[0]/x_scale+ret_map["end"][0]),math.floor(end_control[1]/y_scale+ret_map["end"][1]))

        return ret_map
    
    # 将绘图板数据转换为参数空间的数据，输入的start和end是用来计算参考的，输入的是参数空间的start和end，也就是元数据，不会影响源start和end的值,返回的只有控制点的值是有价值的
    def convert_data_2_point_data(self,source_start,start_control,source_end,end_control):
        ret_map = {}
        height = self.height-self.botton_gap-self.top_gap
        width = self.width-self.left_gap
        y_axis = source_start[1]-source_end[1]
        y_axis_num = abs(y_axis)
        x_axis = source_start[0]-source_end[0]
        x_axis_num = abs(x_axis)
        y_scale = y_axis_num/height
        x_scale = x_axis_num/width
        ...
        ret_map["start_control"]=(math.floor((start_control[0]-ret_map["start"][0])*x_scale),math.floor((start_control[1]-ret_map["start"][1])*y_scale))
        ret_map["end_control"]=(math.floor((end_control[0]-ret_map["end"][0])*x_scale),math.floor((end_control[1]-ret_map["end"][1])*y_scale))

        return ret_map

        # 将parameter points数据更新到参数类中
    def update_parameter_points_2_parameter_map(self):
        if len(self.parameter_points) == 4:
            if self.parameter_points[0] is not None:
                if self.selected_key is not None and self.selected_lable is not None:
                    if len(self.point_frame_index)>0:
                        end_point_frame_index = self.point_frame_index[0]
                        if self.selected_key in self.data.parameter_map:
                            self.data.parameter_map[self.selected_key][self.selected_lable]["control"]["start"] = self.parameter_points[1]
                        if end_point_frame_index in self.data.parameter_map:
                            self.data.parameter_map[end_point_frame_index][self.selected_lable]["control"]["end"] = self.parameter_points[3]
        elif len(self.parameter_points) == 8:
            if self.parameter_points[0] is not None:
                if self.selected_key is not None and self.selected_lable is not None:
                    if len(self.point_frame_index)>1 and self.point_frame_index[0] is not None:
                        end_point_frame_index = self.point_frame_index[0]
                        if self.selected_key in self.data.parameter_map:
                            self.data.parameter_map[self.selected_key][self.selected_lable]["left"]["control"]["start"] = self.parameter_points[1]
                        if end_point_frame_index in self.data.parameter_map:
                            self.data.parameter_map[end_point_frame_index][self.selected_lable]["left"]["control"]["end"] = self.parameter_points[3]
            
            if self.parameter_points[4] is not None:
                if self.selected_key is not None and self.selected_lable is not None:
                    if len(self.point_frame_index)>1 and self.point_frame_index[1] is not None:
                        end_point_frame_index = self.point_frame_index[1]
                        if self.selected_key in self.data.parameter_map:
                            self.data.parameter_map[self.selected_key][self.selected_lable]["right"]["control"]["start"] = self.parameter_points[5]
                        if end_point_frame_index in self.data.parameter_map:
                            self.data.parameter_map[end_point_frame_index][self.selected_lable]["right"]["control"]["end"] = self.parameter_points[7]

    # 将更新后的控制点数据更新到参数类中，并刷新相关的时间轴矩阵数据
    def update_control_point(self):
        if len(self.points)==4:
            if self.points[0] is not None:
                data_map = self.convert_data_2_point_data(source_start=self.parameter_points[0],
                                                          start_control=self.points[1],
                                                          source_end=self.parameter_points[2],
                                                          end_control=self.points[3])
                self.parameter_points[1]=data_map["start_control"]
                self.parameter_points[3]=data_map["end_control"]
            ...
            if self.points[0] is not None:
                ...
                self.parameter_points[1]=data_map["start_control"]
                self.parameter_points[3]=data_map["end_control"]
            if self.points[4] is not None:
                data_map = self.convert_data_2_point_data(source_start=self.parameter_points[4],
                                                          start_control=self.points[5],
                                                          source_end=self.parameter_points[6],
                                                          end_control=self.points[7])
                self.parameter_points[5]=data_map["start_control"]
                self.parameter_points[7]=data_map["end_control"]
        
        self.update_parameter_points_2_parameter_map()
        self.data.update_timeline_data_map_func(self.selected_key)


    # 绘制曲线
    def draw_curve(self,start,start_control,end,end_control):
        self.dc.SetPen(wx.Pen("red", self.point_size))
        # 设置画刷的颜色，用于填充圆形
        self.dc.SetBrush(wx.Brush("red"))
        self.dc.DrawCircle(start[0], start[1], self.point_size)
        self.dc.DrawCircle(end[0], end[1], self.point_size)
        # 创建一个图形路径对象，用于创建贝塞尔曲线
        path = self.gc.CreatePath()
        # 移动到第一个控制点的位置
        path.MoveToPoint(start[0], start[1])
        # 添加一条贝塞尔曲线，使用后三个控制点作为参数
        path.AddCurveToPoint(start_control[0], start_control[1],
                                end_control[0], end_control[1],
                                end[0], end[1])
        # 设置画笔的颜色和宽度
        self.gc.SetPen(wx.Pen("blue", self.line_size))
        # 绘制图形路径，即贝塞尔曲线
        self.gc.DrawPath(path)
        
    # 反转左右并刷新
    def switch_refresh(self):
        self.change_falge = not self.change_falge
        self.on_timeline_draw_left_click()
        self.Refresh()
    
    # 获取关键点的各项参数信息，并保存在point类的全局变量中
    def init_points(self):
        if len(self.data.selected_pointes)>0:
            self.selected_key = self.data.get_selected_point_frame()
            key = self.selected_key
            self.selected_lable = self.data.get_selected_point_lable()
            lable = self.selected_lable

            timeline_list = self.data.parameter_map.keys()
            timeline_list = list(timeline_list)
            timeline_keys = sorted(timeline_list) 

            if key in self.data.parameter_map and lable in self.data.parameter_map[key]:
                key_index = timeline_keys.index(key)
                
                if lable in self.data.morph_lable:
                    left_param = []
                    right_param = []
                    left_convert_point=[]
                    right_convert_point=[]
                    left_end_frame_index = None
                    right_end_frame_index = None

                    if "left" in self.data.parameter_map[key][lable]:
                        lables = [lable,"left"]
                        point_map = self.data.find_back_key(key_parameter_map=self.data.parameter_map,
                                                        parameter_map=self.data.parameter_map,
                                                        key_index=key_index,
                                                        timeline_keys=timeline_keys,
                                                        lables=lables)
                        if bool(point_map):
                            left_end_frame_index = point_map["frame_index"]
                            left_param = [point_map["start"],point_map["start_control"],point_map["end"],point_map["end_control"]]
                            convert_data_map = self.point_data_2_convert_data(start=point_map["start"],start_control=point_map["start_control"],end=point_map["end"],end_control=point_map["end_control"])
                            left_convert_point= [convert_data_map["start"],convert_data_map["start_control"],convert_data_map["end"],convert_data_map["end_control"]]
                    
                    if "right" in self.data.parameter_map[key][lable]:
                        lables = [lable,"right"]
                        point_map = self.data.find_back_key(key_parameter_map=self.data.parameter_map,
                                                        parameter_map=self.data.parameter_map,
                                                        key_index=key_index,
                                                        ...
                                                        lables=lables)
                        if bool(point_map):
                            right_end_frame_index = point_map["frame_index"]
                            right_param = [point_map["start"],point_map["start_control"],point_map["end"],point_map["end_control"]]
                            convert_data_map = self.point_data_2_convert_data(start=point_map["start"],start_control=point_map["start_control"],end=point_map["end"],end_control=point_map["end_control"])
                            right_convert_point= [convert_data_map["start"],convert_data_map["start_control"],convert_data_map["end"],convert_data_map["end_control"]]

                    if left_end_frame_index is not None or right_end_frame_index is not None:
                        self.point_frame_index=[left_end_frame_index,right_end_frame_index]
                    else:
                        self.point_frame_index.clear()

                    if len(left_param)>0 and len(right_param)>0:
                        self.parameter_points = left_param+right_param
                        self.points = left_convert_point+right_convert_point
                    elif len(left_param)==0 and len(right_param)>0:
                        self.parameter_points = [None,None,None,None]+right_param
                        self.points = [None,None,None,None]+right_convert_point
                    elif len(left_param)>0 and len(right_param)==0:
                        self.parameter_points = left_param+[None,None,None,None]
                        self.points = left_convert_point+[None,None,None,None]
                    else:
                        self.parameter_points.clear()
                        self.points.clear()

                elif lable in self.data.non_morph_lable:
                    lables = [lable]
                    # 找到后面的关键点
                    point_map = self.data.find_back_key(key_parameter_map=self.data.parameter_map,
                                                        parameter_map=self.data.parameter_map,
                    ...
                    if bool(point_map):
                        self.point_frame_index = [point_map["frame_index"]]
                        self.parameter_points = [point_map["start"],point_map["start_control"],point_map["end"],point_map["end_control"]]
                        convert_data_map = self.point_data_2_convert_data(start=point_map["start"],start_control=point_map["start_control"],end=point_map["end"],end_control=point_map["end_control"])
                        self.points = [convert_data_map["start"],convert_data_map["start_control"],convert_data_map["end"],convert_data_map["end_control"]]
    ...
    # 当timeline中的点击事件被触发时更新图像
    def on_timeline_draw_left_click(self):
        self.init_points()
        try:
            if self.start_control_dragpoint is not None:
                self.start_control_dragpoint.Destroy()
                self.start_control_dragpoint = None
            if self.end_control_dragpoint is not None:
                self.end_control_dragpoint.Destroy()
                self.end_control_dragpoint = None
        except:
            print("destroy error")
            pass
        ...
        if len(self.points)==4:
            # 创建两个控制点
            self.start_control_dragpoint = DragPoint(self, self, 1)
            self.end_control_dragpoint = DragPoint(self, self, 3)
        elif len(self.points) == 8:
            if self.change_falge:
                self.start_control_dragpoint = DragPoint(self, self, 5)
                self.end_control_dragpoint = DragPoint(self, self, 7)
            else:
                self.start_control_dragpoint = DragPoint(self, self, 1)
                self.end_control_dragpoint = DragPoint(self, self, 3)
    ...
    def OnPaint(self,event):
        ...
        self.dc.SetPen(wx.Pen("black",self.line_size))
        self.dc.SetTextForeground("gray")
        self.dc.SetFont(wx.Font(self.lable_text_size, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
        
        self.gc = wx.GraphicsContext.Create(self.dc) # 曲线绘制
        ...
        if len(self.points)==4:
            if self.points[0] is not None:
                self.draw_curve(self.points[0],self.points[1],self.points[2],self.points[3])

                self.lable_str = self.data.get_selected_point_lable()
            ...
            if self.change_falge:
                if self.points[4] is not None:
                    start = (self.points[4][0],self.points[4][1])
                    start_control = (self.points[5][0],self.points[5][1])
                    end = (self.points[6][0],self.points[6][1])
                    end_control = (self.points[7][0],self.points[7][1])
                elif self.points[0] is not None:
                    self.change_falge=not self.change_falge
                    start = (self.points[0][0],self.points[0][1])
                    start_control = (self.points[1][0],self.points[1][1])
                    end = (self.points[2][0],self.points[2][1])
                    end_control = (self.points[3][0],self.points[3][1])
            else:
                if self.points[0] is not None:
                    start = (self.points[0][0],self.points[0][1])
                    start_control = (self.points[1][0],self.points[1][1])
                    ...
                    end_control = (self.points[3][0],self.points[3][1])
                elif self.points[4] is not None:
                    self.change_falge=not self.change_falge
                    start = (self.points[4][0],self.points[4][1])
                    start_control = (self.points[5][0],self.points[5][1])
                    end = (self.points[6][0],self.points[6][1])
            ...
            if start is not None:
                self.draw_curve(start=start,start_control=start_control,end=end,end_control=end_control)
                # 创建两个控制点
                if self.change_falge:

                    self.lable_str = self.data.get_selected_point_lable()+"_right"
                    
                else:

                    self.lable_str = self.data.get_selected_point_lable()+"_left"
        ...
        self.dc.DrawText(self.lable_str,self.width-self.lable_right_gap,self.top_gap)
...
class ParameterStore:
    def __init__(self):
        ...
        self.selected_pointes = [] # [(key_num,y_lable),(0,"eyebrow")]
    ...
    # 设置选中点
    def set_selected_pointes(self,selected_point):
        self.selected_pointes.clear()
        self.selected_pointes = [selected_point]
    
    # 清空选中点列表
    def clear_selected_pointes(self):
        self.selected_pointes.clear()

    # 获取选中的点的帧
    def get_selected_point_frame(self):
        return self.selected_pointes[0][0]
    
    # 获取选中的点的标签
    def get_selected_point_lable(self):
        return self.selected_pointes[0][1]

    # 初始化时间轴的数据矩阵
    def init_data_map(self):
        self.render_flage_map = [0 for i in range(self.frame_num+1)] # 标记每一帧是否已经有临时渲染
    ...
    # 将坐标值转换为标签
    def find_y_lable_str(self,lable_y):
        if lable_y in self.timeline_lable_map:
            return self.timeline_lable_map[lable_y]
        else:
            return None
        
    # 寻找被选中的标记点，传入的是鼠标点击事件获取的坐标值，时间轴slider长度，slider的前面的空隙
    def find_selected_point(self,click_x,click_y,slider_width,slider_gap):
        selected_point = None
        x_key = None
        y_lable = None

        key_near = (click_x-slider_gap)*self.frame_num//slider_width
        key_near_x = slider_gap+slider_width*(key_near/self.frame_num)
        if abs(click_x-key_near_x)<=self.point_nomal_size:
            x_key = key_near
        else:
            return selected_point

        for i in [k for k in range(click_y-self.point_nomal_size,click_y+self.point_nomal_size+1)]:
            lable_str = self.find_y_lable_str(i)
            if lable_str is not None:
                y_lable = lable_str
                break

        if x_key is not None and y_lable is not None:
            selected_point = (x_key,y_lable)
    ...
    # 删除选中的关键点的数据
    def delete_selected_key_data(self):
        key = self.selected_pointes[0][0]
        lable = self.selected_pointes[0][1]
        if key in self.parameter_map and lable in self.parameter_map[key]:
            self.temp_delete_parameter_map.clear()
            self.temp_delete_parameter_map[key] = {lable:self.parameter_map[key][lable]}
            self.get_selected_point_frame()
            del self.parameter_map[key][lable]
    ...
    def cubic_bezier(self,in_x,start_x,start_y,start_c_x,start_c_y,end_c_x,end_c_y,end_x,end_y):
        ...
        if t is not None:
            ...
            # print("没有找到合适的t值")
            if in_x == end_x:
                y = self.cubic_bezier(in_x-1,start_x,start_y,start_c_x,start_c_y,end_c_x,end_c_y,end_x,end_y)
            elif in_x == start_x:
                y = self.cubic_bezier(in_x+1,start_x,start_y,start_c_x,start_c_y,end_c_x,end_c_y,end_x,end_y)
            else:
                y = self.cubic_bezier(in_x-1,start_x,start_y,start_c_x,start_c_y,end_c_x,end_c_y,end_x,end_y)+self.cubic_bezier(in_x+1,start_x,start_y,start_c_x,start_c_y,end_c_x,end_c_y,end_x,end_y)
                y = y/2
        return y
    ...
    def find_back_key(self,key_parameter_map,parameter_map,key_index,timeline_keys,lables):
        ...
        if lables[0] in self.morph_lable:
            ...
            if not bool(ret_map):
                type = key_parameter_map[current_key][lables[0]]["type"]
                end = (self.frame_num,key_parameter_map[current_key][lables[0]][lables[1]]["value"])
                end_control = (0,0)
                start = (current_key,key_parameter_map[current_key][lables[0]][lables[1]]["value"])
                start_control = (0,0)
                ret_map = {
            ...
            if not bool(ret_map):
                end = (self.frame_num,key_parameter_map[current_key][lables[0]]["value"])
                end_control = (0,0)
                start = (current_key,key_parameter_map[current_key][lables[0]]["value"])
                start_control = (0,0)
                ret_map = {
    ...
    # 判断是否是删除点
    def is_delete_point(self):
        if len(self.selected_pointes)>0 and self.selected_pointes[0][0] in self.parameter_map and self.selected_pointes[0][1] not in self.parameter_map[self.selected_pointes[0][0]]:
            return True
        else:
            return False
    ...
    # 更新指定关键帧的相邻控制面板矩阵数据
    def update_timeline_data_map_func(self,key):
        function_map = self.find_keys(key=key)
    ...
    def update_timeline_data_map(self):
        ...
        self.update_timeline_data_map_func(key=key)

    # 更新面板上所有的关键点的相关矩阵数据
    def update_all_timeline_data_map(self):
        for key in self.parameter_map.keys():
            self.update_timeline_data_map_func(key=key)
...
class MainFrame(wx.Frame):
    def __init__(self, poser: Poser, device: torch.device):
        ...
        self.fps = 30 # 帧率和界面刷新频率
        self.isrendered = False # 是否已经渲染过了
        self.isrendering = False # 是否正在渲染
        self.render_path = None # 渲染输出文件夹
        self.render_filename = None # 渲染文件夹中输出的文件名头部
        self.slider_event_flage = False # slider 事件标记
        self.preset_default_path = "./preset" # 默认预设文件夹的位置
        self.preset_name_set = []   # 预设文件集合
        ...
        self.scroll_panel = wx.lib.scrolledpanel.ScrolledPanel(self, -1)
        ...
        self.main_panel = wx.Panel(self.scroll_panel, style=wx.NO_BORDER)
        ...
        self.scroll_panel_sizer.Add(self.main_panel,0,wx.EXPAND)

        self.init_botton_panel()
        self.scroll_panel_sizer.Fit(self.scroll_panel)

        # self.big_main_sizer.Add(self.scroll_panel, 0, wx.CENTER)
        ...
        # 点击渲染后的timer，用于播放时间轴上的关键点
        self.render_timer = wx.Timer(self, wx.ID_ANY)
        self.Bind(wx.EVT_TIMER, self.on_render_timer, self.render_timer)

        save_image_id = wx.NewIdRef()
        self.Bind(wx.EVT_MENU, self.on_save_preset, id=save_image_id)
    ...
    def init_botton_panel(self):
        ...
        self.botton_panel = wx.Panel(self.scroll_panel, style=wx.SIMPLE_BORDER, size=(botton_width, height_size))
        ...
        self.button_delete.Bind(wx.EVT_BUTTON, self.delete_selected_key_data)
        ...
        self.scroll_panel_sizer.Add(self.botton_panel, 0, wx.CENTER)
    ...
    def init_left_panel(self):
        ...
        # 添加曲线绘制面板
        self.curve_panel = CurvePlottingPanel(self.left_panel,self.data)
    ...
    def init_right_panel(self):
        ...
        # 添加预设下拉选择框
        if os.path.exists(self.preset_default_path):
            self.preset_name_set = [f for f in os.listdir(self.preset_default_path) if f.endswith('.json')]
        else:
            os.mkdir(self.preset_default_path)

        self.preset_choice = wx.Choice(self.right_panel, choices=["No preset selected"]+self.preset_name_set,size=(-1,7))
        right_panel_sizer.Add(self.preset_choice, 1, wx.EXPAND)
        self.preset_choice.SetSelection(0)
        self.preset_choice.Bind(wx.EVT_CHOICE, self.on_choice_preset) # 绑定一个选择事件
        ...
        self.preset_save_button.Bind(wx.EVT_BUTTON, self.on_save_preset)
        self.preset_load_button.Bind(wx.EVT_BUTTON, self.on_load_preset)
        ...
        self.colon_static_text = wx.StaticText(self.time_jump_panel, label=" SECOND : FRAME ", style=wx.ALIGN_CENTER)
        ...
        self.goto_button.Bind(wx.EVT_BUTTON, self.on_goto_button_clicked)
        ...
        self.render_button.Bind(wx.EVT_BUTTON, self.on_render)
    ...
    def on_switch_button(self,event):
        self.curve_panel.switch_refresh()

    # 删除当前选中关键点的数据
    def delete_selected_key_data(self, event):
        self.data.delete_selected_key_data()
        self.data.update_timeline_data_map()
        self.Refresh()
    ...
    def OnSize(self, event):
        self.scroll_panel.SetSize(self.GetSize())
        event.Skip()
    ...
    # 渲染定时器
    def on_render_timer(self,event):
        key = -1
        if self.isrendering :
            key = self.data.get_now_key_value()
        else:
        ...
        keys = sorted(list(self.data.parameter_map.keys()))
        if len(keys)>0:
            end_key = keys[len(keys)-1]
            if key < end_key:
                key_in = key+1
                image_file_name = os.path.join(self.render_path,self.render_filename+"_"+str(key_in).zfill(4)+'.png')
                self.botton_timeline_panel.timeline_slider.SetValue(key_in)
                wx.PostEvent(self.botton_timeline_panel.timeline_slider, wx.CommandEvent(wx.EVT_SLIDER.typeId, self.botton_timeline_panel.timeline_slider.GetId()))
                ...
                self.render_button.SetLabel("render")
    ...
    # # 渲染函数
    # def rendering(self,file_dir,file_name):

    #     self.render_button.SetLabel("rendering...")
    #     key_list = list(self.data.parameter_map.keys())
    #     key_list.sort()
    #     if len(key_list)>0:
    #         start = key_list[0]
    #         end = key_list[len(key_list)-1]
    #         for i in range(start,end+1):
    #             image_file_name = os.path.join(file_dir,file_name+"_"+str(i).zfill(4)+".png")
                
    #             self.slider_event_flage=False
                
    #             self.botton_timeline_panel.timeline_slider.SetValue(i)
                
    #             wx.PostEvent(self.botton_timeline_panel.timeline_slider, wx.CommandEvent(wx.EVT_SLIDER.typeId, self.botton_timeline_panel.timeline_slider.GetId()))
                
    #             while not self.slider_event_flage:
    #                 time.sleep(2)
    ...
    # 读取预设文件数据
    def load_preset_data(self,path):
        # 先清空
        self.data.parameter_map = {}
        self.data.init_data_map()
        if os.path.exists(path):
            with open(path, 'r') as f:
                data = json.load(f)
                for k,v in data.items():
                    self.data.parameter_map[int(k)] = v

    # 导出关键帧数据为json文件
    def save_preset_data(self,path):
        with open(path, 'w') as f:
            json.dump(self.data.parameter_map, f)

        message_dialog = wx.MessageDialog(self, f"Saved {path}", "Manual Poser", wx.OK)
        message_dialog.ShowModal()
        message_dialog.Destroy()

    # 预设保存按钮回调
    def on_save_preset(self,event):
        if self.preset_choice.GetSelection() == 0:
            file_dialog = wx.FileDialog(self, "Choose an preset file", self.preset_default_path, "", "*.json", wx.FD_SAVE)
            if file_dialog.ShowModal() == wx.ID_OK:
                dir = file_dialog.GetDirectory()
                name = file_dialog.GetFilename()
                file_name = os.path.join(dir, name)
                try:
                    self.save_preset_data(file_name)
                except:
                    message_dialog = wx.MessageDialog(self, f"Could not save {file_name}", "Manual Poser", wx.OK)
                    message_dialog.ShowModal()
                    message_dialog.Destroy()
                if name not in self.preset_name_set:
                    self.preset_name_set.append(name)
                    self.preset_choice.Append(name)
                self.preset_choice.SetSelection(self.preset_name_set.index(name)+1)
            file_dialog.Destroy()
        else:
            str = self.preset_choice.GetStringSelection()
            file_name = os.path.join(self.preset_default_path, str)
            try:
                self.save_preset_data(file_name)
            except:
                message_dialog = wx.MessageDialog(self, f"Could not save {file_name}", "Manual Poser", wx.OK)
    ...
    def on_load_preset(self,event):
        file_dialog = wx.FileDialog(self, "Choose an preset file", self.preset_default_path, "", "*.json", wx.FD_OPEN)
        if file_dialog.ShowModal() == wx.ID_OK:
            dir = file_dialog.GetDirectory()
            name = file_dialog.GetFilename()
            file_name = os.path.join(dir, name)
            new_file_name = os.path.join(self.preset_default_path, name)
            shutil.copyfile(file_name, new_file_name)
            try:
                self.load_preset_data(new_file_name)
                self.data.update_all_timeline_data_map()
            except:
                pass
    ...
    # 选择预设事件
    def on_choice_preset(self,event):
        if self.preset_choice.GetSelection() > 0:
            str = self.preset_choice.GetStringSelection()
            file_name = os.path.join(self.preset_default_path, str)
            try:
                self.load_preset_data(file_name)
                self.data.update_all_timeline_data_map()
            except:
                pass
        else:
    ...
    # 跳转按钮回调事件
    def on_goto_button_clicked(self,event):
        sec_text = self.s_text_ctrl.GetValue()
        frame_text = self.z_text_ctrl.GetValue()
        if sec_text == '':
            sec_text = str(0)
        if frame_text == '':
            frame_text = str(0)
        ...
        frame = sec*self.fps+frame
        if frame>self.data.frame_num:
            frame = self.data.frame_num
        self.botton_timeline_panel.timeline_slider.SetValue(frame)
        wx.PostEvent(self.botton_timeline_panel.timeline_slider, wx.CommandEvent(wx.EVT_SLIDER.typeId, self.botton_timeline_panel.timeline_slider.GetId()))

    # 清空文件夹
    def clear_floder(self,dir_name):
    ...
    def on_render(self,event):
        ...
        if self.render_path is not None:
            dir_name = self.render_path
        else:
            dir_name = "data/images"
        file_dialog = wx.FileDialog(self, "Choose an image", dir_name, "", "*.png", wx.FD_SAVE)
        if file_dialog.ShowModal() == wx.ID_OK:
            in_dir_name = file_dialog.GetDirectory()
            try:
                if len(os.listdir(file_dialog.GetDirectory())) > 0:
                    message_dialog = wx.MessageDialog(self, f"The {in_dir_name} is not empty,do you want to override it?", "Manual Poser",
                                                      wx.YES_NO | wx.ICON_QUESTION)
                    result = message_dialog.ShowModal()
                    if result == wx.ID_YES:
                        self.render_path = in_dir_name
                        self.render_filename = file_dialog.GetFilename().split(".")[0]
                        message_dialog.Destroy()
                        self.clear_floder(in_dir_name) # 清空文件夹
                        self.timer.Stop()
                        self.render_timer.Start(1000//self.fps)
                        self.render_button.SetLabel("rendering...")
                    else:
                        message_dialog.Destroy()
                else:
                ...
                message_dialog = wx.MessageDialog(self, f"Could not save {in_dir_name}", "Manual Poser", wx.OK)
                message_dialog.ShowModal()
                message_dialog.Destroy()
